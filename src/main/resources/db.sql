...
set address=?
where user_id=?;
select u.id, u.type, concat(e.first_name, c.company_name) as name
from users u
...
select p.id,
       ...
       string_agg(level_name, ',/ ') as levels
from posts p
...
having s.skill_name like ?
   and c.company_name like ?
   and string_agg(level_name, ',/') like ?
   and jt.type_name like ?;
